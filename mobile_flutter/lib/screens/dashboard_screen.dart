...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      setState(() {
        ...
        if (evs is Map && evs['results'] is List)
          _events = evs['results'] as List;
        _loading = false;
      });
    } catch (_) {
  ...
  Future<void> _generateWeek() async {
    ...
    try {
      ...
      if (result['status'] == 'completed' ||
          (result['output'] is Map && result['output']['status'] == 'ok')) {
        final fresh = await outfitsApi.weekly();
        setState(() => _weekDays = fresh is List ? fresh : []);
      } else {
        setState(
          () => _error =
              (result['error'] ?? result['output']?['message'] ?? 'Failed')
                  .toString(),
        );
      }
    } catch (e) {
    ...
    try {
      ...
      setState(() {
        _weekDays = _weekDays.map((d) {
          if (d is Map &&
              d['recommendation'] is Map &&
              d['recommendation']['id'] == recId) {
            return {
              ...Map<String, dynamic>.from(d),
              'recommendation': Map<String, dynamic>.from(updated),
            };
          }
          return d;
        }).toList();
  ...
  Widget build(BuildContext context) {
    ...
    final hasAnyRec = _weekDays.any(
      (d) => d is Map && d['recommendation'] != null,
    );
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          const Text(
            'Your weekly outfit plan — unique looks for every day.',
            ...
            SizedBox(
              ...
              child: ListView.separated(
                ...
                itemBuilder: (context, idx) {
                  ...
                  final accepted = hasRec
                      ? (day['recommendation'] as Map)['accepted']
                      : null;
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            isToday
                                ? 'Today'
                                : (day['day_label']?.toString().substring(
                                        0,
                                        3,
                                      ) ??
                                      ''),
...
class _DayDetailCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final rec = day['recommendation'] is Map
        ? Map<String, dynamic>.from(day['recommendation'] as Map)
        : null;
    ...
    final weather = rec?['weather_snapshot'] is Map
        ? Map<String, dynamic>.from(rec!['weather_snapshot'] as Map)
        : null;
    final dayEvents = events
        .where((e) => (e['start_time'] as String?)?.startsWith(dateVal) == true)
        .toList();
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      '${weather['condition']} · ${weather['precipitation_probability'] ?? 0}% rain',
                      style: const TextStyle(
                        color: AppColors.creamDim,
                        fontSize: 12,
                      ),
                    ...
                    child: Row(
                      children: [
                        ...
                        if (ev['formality'] != null)
                          ABadge(
                            text: ev['formality'].toString().replaceAll(
                              '_',
                              ' ',
                            ),
                          ),
          ...
          if (rec == null)
            const Center(
              child: Padding(
                ...
                child: Text(
                  'No outfit for this day yet. Generate the full week.',
                  style: TextStyle(color: AppColors.creamDim, fontSize: 13),
                ),
              ),
            )
