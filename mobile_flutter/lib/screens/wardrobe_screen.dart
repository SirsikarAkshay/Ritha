  ...
  Future<void> _delete(int id, String name) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text(
          'This will delete it from your wardrobe.',
          ...
          child: Column(
            ...
            children: [
              ListTile(
                ...
                subtitle: const Text(
                  'Enter item details or snap a photo',
              ...
              ListTile(
                ...
                subtitle: const Text(
                  'Paste a shopping email to auto-add items',
                  style: TextStyle(color: AppColors.creamDim, fontSize: 12),
                ),
                onTap: () {
                  Navigator.pop(ctx);
    ...
    try {
      ...
      setState(() {
        ...
        if (res['material'] != null)
          _material.text = res['material'].toString();
        if (res['category'] != null && _categories.contains(res['category']))
          _category = res['category'].toString();
        if (res['formality'] != null && _formalities.contains(res['formality']))
          _formality = res['formality'].toString();
        if (res['season'] != null && _seasons.contains(res['season']))
          _season = res['season'].toString();
        if (res['colors'] is List)
          _colors.text = (res['colors'] as List).join(', ');
        _analyzing = false;
      });
    } catch (e) {
      if (mounted)
...
class _ReceiptImportSheetState extends State<_ReceiptImportSheet> {
  ...
  Future<void> _parseReceipt() async {
    ...
    if (text.isEmpty) {
      setState(() => _error = 'Paste your receipt email text above.');
      return;
    }
    setState(() {
      _parsing = true;
    ...
    try {
      ...
      if (res['error'] != null) {
        setState(() {
          _error = res['error']['message']?.toString() ?? 'Parse failed';
          _parsing = false;
        });
        return;
      }
      ...
      if (items.isEmpty) {
        setState(() {
          _error = 'No clothing items found in this receipt.';
          _parsing = false;
        });
        return;
      }
      ...
      if (mounted)
        setState(() {
          _error = 'Failed to parse receipt: $e';
          _parsing = false;
        });
    }
  }
  ...
  Future<void> _saveSelected() async {
    ...
    if (toSave.isEmpty) {
      setState(() => _error = 'Select at least one item.');
      return;
    }
    setState(() {
      _saving = true;
          ...
          child: Column(
            ...
            children: [
              ...
              if (_parsed == null) ...[
                ...
                Text(
                  '${_parsed!.length} item${_parsed!.length == 1 ? '' : 's'} found — select which to add:',
                  style: const TextStyle(
                    color: AppColors.cream,
                    fontSize: 14,
                  ...
                  children: [
                    ...
                    Expanded(
                      child: APrimaryButton(
                        label:
                            'Add ${_selected.where((s) => s).length} item${_selected.where((s) => s).length == 1 ? '' : 's'}',
                        loading: _saving,
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: selected
                ? AppColors.terra.withValues(alpha: 0.5)
                : AppColors.border,
          ),
        ),
        child: Row(
