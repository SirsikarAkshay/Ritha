    ...
    _sub = _ws!.onMessage.listen((data) {
      ...
      setState(() {
        if (eventType == 'item_added' && payload is Map) {
          if (!_items.any((i) => i['id'] == payload['id']))
            _items.add(Map<String, dynamic>.from(payload));
          ...
          _wardrobe = {
            ..._wardrobe!,
            'members': members
                .where((m) => m['user']?['id'] != payload['user_id'])
                .toList(),
          };
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        leading: IconButton(
          onPressed: () => context.go('/shared-wardrobes'),
          ...
          children: [
            ...
            if (_items.isEmpty)
              const EmptyState(
                icon: '📦',
                title: 'No items yet',
                body: 'Add the first one with the + button.',
              ),
                  ...
                  child: Row(
                    children: [
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              '${item['category']} · by @${item['added_by']?['handle'] ?? ''}',
                              style: const TextStyle(
                                color: AppColors.creamDim,
                                fontSize: 12,
                              ),
                      ...
                      if (isOwner || item['added_by']?['id'] == userId) ...[
                        IconButton(
                          onPressed: () =>
                              _editItem(Map<String, dynamic>.from(item as Map)),
                          icon: const Icon(
                            Icons.edit_outlined,
                            color: AppColors.creamDim,
                            size: 20,
        ...
        child: Column(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              style: const TextStyle(color: AppColors.cream),
              items: [
                for (final c in _sharedCategories)
...
class _EditSharedItemSheetState extends State<_EditSharedItemSheet> {
  ...
  Future<void> _save() async {
    ...
    try {
      final updated = await sharedWardrobesApi.items
          .update(widget.wardrobeId, widget.item['id'] as int, {
            'name': _name.text.trim(),
            'category': _category,
            'brand': _brand.text.trim(),
            'notes': _notes.text.trim(),
