...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  Future<void> _loadPreview() async {
    ...
    try {
      final res = await wardrobeApi.starterPackPreview(
        region: _region,
        gender: _gender,
      );
      setState(() {
  ...
  List<dynamic> get _visibleItems {
    ...
    return items.where((it) {
      ...
      if (it['is_opt_in'] == true && _optIns.contains(it['opt_in_group']))
        return true;
      return false;
    }).toList();
  }
  ...
  List<Widget> _buildStep1() => [
        ...
        options: _regions
            .map<Map<String, String>>(
              (r) => {
                ...
                'hint':
                    _REGION_HINTS[r['code']] ??
                    (r['climate_zone'] as String? ?? ''),
              },
            )
            .toList(),
        selected: _region,
    ...
    _FooterBar(
      ...
      rightLabel: 'Continue',
      onRight: (_gender != null && _region != null)
          ? () => setState(() => _step = 2)
          : null,
    ),
  ];
          ...
          child: InkWell(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(12),
                border: Border.all(
                  color: isSel ? AppColors.terra : AppColors.surface3,
                ),
              ),
              child: Column(
                children: [
...
class _ItemCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final season = item['seasonality'] == 'all'
        ? 'all season'
        : item['seasonality'];
    return Opacity(
            ...
            Wrap(
              ...
              children: added.asMap().entries.map((e) {
                return Chip(
                  backgroundColor: AppColors.surface2,
                  label: Text(
                    '${_CAT_ICON[e.value['category']] ?? ''} ${e.value['name']}',
                    style: const TextStyle(
                      color: AppColors.cream,
                      fontSize: 13,
                    ),
