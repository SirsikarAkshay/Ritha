          ...
          Padding(
            ...
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 4),
                Text(
                  'Find friends by handle. Connect, chat, plan trips together.',
...
class _ProfilePanelState extends State<_ProfilePanel> {
  ...
  Future<void> _save() async {
    ...
    try {
      final updated =
          await socialApi.profile.update({
      ...
      if (_handle.text.trim() != _profile!['handle']) {
        final afterHandle =
            await socialApi.profile.updateHandle(
  ...
  Widget build(BuildContext context) {
    if (_profile == null) {
      return const Padding(
        ...
        child: Text('Loading…', style: TextStyle(color: AppColors.creamDim)),
      );
    }
    if (!_editing) {
      return Padding(
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      _profile!['display_name'] ?? _profile!['handle'] ?? '',
...
class _FindTabState extends State<_FindTab> {
  ...
  Future<void> _search() async {
    ...
    try {
      final res =
          await socialApi.users.search(
                _handle.text.trim().toLowerCase().replaceAll('@', ''),
              )
              as Map;
            ...
            child: Row(
              children: [
                ...
                if (_result!['is_self'] == true)
                  ...
                  _ConnectionCta(
                    connection: _result!['user']['connection'],
  ...
  Widget build(BuildContext context) {
    if (connection == null) {
      return ElevatedButton(
        ...
        child: Text(busy ? 'Sending…' : 'Connect'),
      );
    }
    final status = connection['status'];
    final direction = connection['direction'];
    if (status == 'accepted')
      return const Text('Connected ✓', style: TextStyle(color: AppColors.sage));
    ...
    if (status == 'pending' && direction == 'incoming')
      return const Text(
        'In Requests tab',
        style: TextStyle(color: AppColors.terraLight),
      );
    return ElevatedButton(
      onPressed: busy ? null : onConnect,
              ...
              child: Row(
                children: [
                  ...
                  if (widget.withActions &&
                    ...
                    TextButton(
                      onPressed: () => _act(
                        () => socialApi.connections.accept(conn['id'] as int),
                      ),
                      child: const Text('Accept'),
                    ),
                    TextButton(
                      onPressed: () => _act(
                        () => socialApi.connections.reject(conn['id'] as int),
                      ),
                  ...
                  if (widget.withActions && conn['direction'] == 'outgoing')
                    TextButton(
                      onPressed: () => _act(
                        () => socialApi.connections.remove(conn['id'] as int),
                      ),
                      style: TextButton.styleFrom(
                        foregroundColor: AppColors.danger,
                      ),
                  ...
                  if (!widget.withActions && widget.status == 'accepted')
                    IconButton(
                      onPressed: () =>
                          context.go('/messages?open_user_id=${other['id']}'),
                      icon: const Icon(
