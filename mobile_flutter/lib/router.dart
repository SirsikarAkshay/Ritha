...
GoRouter buildRouter(AuthProvider auth) {
  return GoRouter(
    ...
    routes: [
      ...
      GoRoute(path: '/reset-password', builder: (ctx, st) => ResetPasswordScreen(email: st.uri.queryParameters['email'] ?? '')),
      GoRoute(path: '/verify-email',   builder: (ctx, st) => VerifyEmailScreen(email: st.uri.queryParameters['email'] ?? '')),
      ...
      ShellRoute(
        ...
        routes: [
          ...
          GoRoute(path: '/trips',            builder: (_, __) => const TripPlannerScreen()),
          GoRoute(path: '/cultural',         builder: (_, __) => const CulturalScreen()),
          GoRoute(path: '/sustainability',   builder: (_, __) => const SustainabilityScreen()),
          GoRoute(path: '/outfit-history',   builder: (_, __) => const OutfitHistoryScreen()),
          GoRoute(path: '/profile',          builder: (_, __) => const ProfileScreen()),
          GoRoute(path: '/people',           builder: (_, __) => const PeopleScreen()),
          GoRoute(path: '/messages',         builder: (ctx, st) {
            final openWith = int.tryParse(st.uri.queryParameters['open_user_id'] ?? '');
            return MessagesScreen(openWithUserId: openWith);
          }),
          GoRoute(path: '/shared-wardrobes', builder: (_, __) => const SharedWardrobesScreen()),
          GoRoute(
            path: '/shared-wardrobes/:id',
            builder: (ctx, st) => SharedWardrobeDetailScreen(id: int.parse(st.pathParameters['id']!)),
          ),
        ],
      ),
    ],
