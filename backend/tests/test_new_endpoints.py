...
class TestPushToken:
    ...
    def test_update_token(self, client):
        user = UserFactory()
        h = auth(client, user)
        client.post("/api/auth/push-token/", {"token": "old-token"}, content_type="application/json", **h)
        client.post("/api/auth/push-token/", {"token": "new-token"}, content_type="application/json", **h)
        user.refresh_from_db()
        assert user.device_push_token == "new-token"

    def test_requires_authentication(self, client):
        r = client.post("/api/auth/push-token/", {"token": "x"}, content_type="application/json")
