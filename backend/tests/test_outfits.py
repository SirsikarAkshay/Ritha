...
class TestDailyRecommendationEndpoint:
    ...
    def test_recommendation_idempotent(self, client):
        ...
        client.post("/api/agents/daily-look/", weather_payload, content_type="application/json", **h)
        client.post("/api/agents/daily-look/", weather_payload, content_type="application/json", **h)
...
class TestOutfitFeedback:
    ...
    def test_accept_recommendation(self, client):
        ...
        h = auth_header(client, user)
        r = client.patch(
            f"/api/outfits/recommendations/{rec.id}/feedback/", {"accepted": True}, content_type="application/json", **h
        )
        assert r.status_code == 200
    ...
    def test_reject_recommendation(self, client):
        ...
        h = auth_header(client, user)
        r = client.patch(
            f"/api/outfits/recommendations/{rec.id}/feedback/",
            {"accepted": False},
            content_type="application/json",
            **h,
        )
    ...
    def test_feedback_missing_field(self, client):
        ...
        rec = self._make_recommendation(user)
        h = auth_header(client, user)
        r = client.patch(f"/api/outfits/recommendations/{rec.id}/feedback/", {}, content_type="application/json", **h)
        assert r.status_code == 400
