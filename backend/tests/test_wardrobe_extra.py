...
class TestReceiptImport:
    def test_missing_email_body_returns_400(self, client):
        ...
        r = client.post("/api/wardrobe/receipt-import/", {}, content_type="application/json", **h)
...
class TestLuggageWeight:
    def test_no_items_returns_400(self, client):
        user = UserFactory()
        h = auth_header(client, user)
        r = client.post("/api/wardrobe/luggage-weight/", {"item_ids": []}, content_type="application/json", **h)
        assert r.status_code == 400
    ...
    def test_estimated_weight_used_when_null(self, client):
        ...
        h = auth_header(client, user)
        r = client.post("/api/wardrobe/luggage-weight/", {"item_ids": [item.id]}, content_type="application/json", **h)
        assert r.status_code == 200
    ...
    def test_carry_on_limit_exceeded(self, client):
        user = UserFactory()
        heavy = [
            ClothingItemFactory(user=user, category="outerwear", weight_grams=2000) for _ in range(6)
        ]  # 12 kg total
        h = auth_header(client, user)
        r = client.post(
            "/api/wardrobe/luggage-weight/",
...
class TestReceiptImportLivePath:
    ...
    def test_live_path_creates_items(self, client, settings):
        ...
        with patch(
            "ritha.services.mistral_client.chat_json",
            return_value={
                "items": [
                    {
            ...
            r = client.post(
                "/api/wardrobe/receipt-import/",
                {"email_body": "Order confirmed: 1x Blue Denim Jacket, 1x White T-Shirt", "auto_save": True},
                content_type="application/json",
                **h,
            )
