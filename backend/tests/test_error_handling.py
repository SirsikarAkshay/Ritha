...
class TestErrorEnvelope:
    ...
    def test_validation_error_has_envelope(self, client):
        r = client.post("/api/auth/register/", {"email": "bad"}, content_type="application/json")
    ...
    def test_wrong_credentials_has_envelope(self, client):
        r = client.post(
            "/api/auth/login/", {"email": "nobody@x.com", "password": "wrong"}, content_type="application/json"
        )
        assert r.status_code == 401
        data = r.json()
        assert "error" in data
...
class TestAgentInputValidation:
    ...
    def test_cultural_advisor_missing_country(self, client):
        user = UserFactory()
        h = self._auth(client, user)
        r = client.post("/api/agents/cultural-advisor/", {}, content_type="application/json", **h)
        assert r.status_code == 400
...
class TestAgentFailurePath:
    ...
    def test_agent_failure_returns_500_envelope(self, client, monkeypatch):
        ...
        r = client.post("/api/agents/conflict-detector/", {"date": "2026-04-01"}, content_type="application/json", **h)
    ...
    def test_failed_agent_job_recorded(self, client, monkeypatch):
        ...
        monkeypatch.setattr(services, "run_conflict_detector", lambda u, d: (_ for _ in ()).throw(ValueError("bad")))
        client.post("/api/agents/conflict-detector/", {"date": "2026-04-01"}, content_type="application/json", **h)
...
class TestModelStringRepresentations:
    ...
    def test_sustainability_log_str(self):
        ...
        user = UserFactory()
        log = SustainabilityLog.objects.create(user=user, action="wear_again", points=10)
    ...
    def test_agent_job_str(self):
        ...
        user = UserFactory()
        job = AgentJob.objects.create(user=user, agent_type="daily_look", status="pending")
