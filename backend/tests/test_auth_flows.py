...
class TestLoginErrors:
    def test_wrong_password_returns_401_not_500(self, client):
        ...
        r = client.post(
            "/api/auth/login/", {"email": user.email, "password": "completely-wrong"}, content_type="application/json"
        )
    ...
    def test_unknown_email_returns_401_not_500(self, client):
        r = client.post(
            "/api/auth/login/", {"email": "nobody@nowhere.com", "password": "anything"}, content_type="application/json"
        )
        assert r.status_code == 401
...
class TestRegisterErrors:
    ...
    def test_duplicate_email_returns_400_not_500(self, mock_send, client):
        user = UserFactory()
        r = client.post(
            "/api/auth/register/", {"email": user.email, "password": "newpassword99"}, content_type="application/json"
        )
    ...
    def test_short_password_returns_400_with_message(self, client):
        r = client.post(
            "/api/auth/register/", {"email": "new@test.com", "password": "short"}, content_type="application/json"
        )
        assert r.status_code == 400
        assert r.json()["error"]["code"] == "validation_error"

    def test_missing_email_returns_400(self, client):
        r = client.post("/api/auth/register/", {"password": "testpass99"}, content_type="application/json")
        assert r.status_code == 400

    @patch("auth_app.views.send_verification_email", return_value=True)
    def test_valid_register_returns_201(self, mock_send, client):
        r = client.post(
            "/api/auth/register/", {"email": "brand@new.com", "password": "goodpass99"}, content_type="application/json"
        )
    ...
    def test_invalid_email_format_returns_400(self, client):
        r = client.post(
            "/api/auth/register/", {"email": "not-an-email", "password": "testpass99"}, content_type="application/json"
        )
        assert r.status_code == 400
...
class TestForgotPassword:
    ...
    def test_sends_reset_email_for_known_address(self, mock_send, client):
        user = UserFactory()
        r = client.post("/api/auth/forgot-password/", {"email": user.email}, content_type="application/json")
        assert r.status_code == 200
        assert mock_send.called

    def test_unknown_email_returns_200_anyway(self, client):
        """Never reveal whether an email is registered."""
        r = client.post("/api/auth/forgot-password/", {"email": "ghost@nowhere.com"}, content_type="application/json")
        assert r.status_code == 200
    ...
    def test_inactive_user_silently_ignored(self, mock_send, client):
        ...
        client.post("/api/auth/forgot-password/", {"email": user.email}, content_type="application/json")
...
class TestResetPassword:
    ...
    def test_can_login_after_reset(self, client):
        ...
        r = client.post(
            "/api/auth/login/", {"email": user.email, "password": "BrandNew99!"}, content_type="application/json"
        )
        assert r.status_code == 200
