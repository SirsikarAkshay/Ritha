...
class TestGoogleConnect:
    ...
    @patch(
        "calendar_sync.google_calendar.get_authorization_url",
...
class TestGoogleSync:
    ...
    @patch(
        "calendar_sync.google_calendar.sync_events",
    ...
    @patch("calendar_sync.google_calendar.sync_events", return_value={"error": "Token expired"})
...
class TestAppleConnect:
    ...
    @patch("calendar_sync.apple_calendar.test_connection", return_value=(True, "Connected — 3 calendars found."))
    ...
    @patch("calendar_sync.apple_calendar.test_connection", return_value=(False, "Authentication failed."))
...
class TestAppleSync:
    ...
    @patch("calendar_sync.apple_calendar.sync_events", return_value={"created": 8, "updated": 2, "errors": []})
    def test_sync_returns_counts(self, mock_sync, client):
        user = make_apple_user()
        h = auth(client, user)
        r = client.post("/api/calendar/apple/sync/", **h)
...
class TestPasswordEncryption:
    def test_password_encrypted_at_rest(self):
        from calendar_sync.apple_calendar import _decrypt_password, save_credentials
    ...
    def test_credentials_retrievable(self):
        from calendar_sync.apple_calendar import get_credentials, save_credentials
...
class TestSyncCalendarsCommand:
    @patch("calendar_sync.google_calendar.sync_events", return_value={"created": 5, "updated": 1, "errors": []})
    ...
    def test_command_skips_unconnected_users(self):
        ...
        with (
            patch("calendar_sync.google_calendar.sync_events") as mock_g,
...
class TestItinerarySyncWithCalendars:
    ...
    @patch("calendar_sync.apple_calendar.sync_events", return_value={"created": 3, "updated": 0, "errors": []})
...
class TestGoogleWebhook:
    ...
    @patch("calendar_sync.google_calendar.sync_events", return_value={"created": 2, "updated": 0, "errors": []})
