...
class TestLogout:
    ...
    def test_blacklisted_refresh_token_rejected(self, client):
        ...
        r = client.post("/api/auth/refresh/", {"refresh": tokens["refresh"]}, content_type="application/json")
    ...
    def test_logout_missing_refresh_returns_400(self, client):
        ...
        tokens = get_tokens(client, user)
        r = client.post(
            "/api/auth/logout/", {}, content_type="application/json", HTTP_AUTHORIZATION=f"Bearer {tokens['access']}"
        )
        assert r.status_code == 400

    def test_logout_requires_authentication(self, client):
        r = client.post("/api/auth/logout/", {"refresh": "x"}, content_type="application/json")
        assert r.status_code == 401
...
class TestPasswordChange:
    def test_change_password_success(self, client):
        ...
        r2 = client.post(
            "/api/auth/login/", {"email": user.email, "password": "newSecure123"}, content_type="application/json"
        )
...
class TestDeleteAccount:
    def test_delete_removes_user(self, client):
        ...
        uid = user.id
        tokens = get_tokens(client, user)
        r = client.delete("/api/auth/me/delete/", HTTP_AUTHORIZATION=f"Bearer {tokens['access']}")
    ...
    def test_delete_cascades_wardrobe(self, client):
        ...
        item_id = item.id
        tokens = get_tokens(client, user)
        client.delete("/api/auth/me/delete/", HTTP_AUTHORIZATION=f"Bearer {tokens['access']}")
...
class TestPasswordChangeLoginVerification:
    ...
    def test_new_password_works_after_change(self, client):
        ...
        r = client.post(
            "/api/auth/login/", {"email": user.email, "password": "newSecure999"}, content_type="application/json"
        )
        assert r.status_code == 200
