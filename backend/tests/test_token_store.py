...
class TestEncryptDecrypt:
    def test_encrypt_produces_non_json_string(self):
        ...
        # The encrypted blob must not be readable as the original plain JSON:
        # either json.loads fails outright, or it parses to something that no
        # longer carries the plaintext token.
        try:
            parsed = json.loads(encrypted)
        except (json.JSONDecodeError, ValueError):
            parsed = None
        assert not (isinstance(parsed, dict) and parsed.get("token") == "abc"), "Token was stored as plain JSON"

    def test_decrypt_round_trips_correctly(self):
        from calendar_sync.token_store import decrypt_tokens, encrypt_tokens
...
class TestStoreLoadGoogle:
    def test_store_and_load_google_tokens(self):
        from calendar_sync.token_store import load_google_tokens, store_google_tokens
