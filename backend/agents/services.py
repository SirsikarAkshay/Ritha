...
def _explain_outfit(
    ...
    if trip:
        ...
        bullets.append(
            {"type": "trip", "text": f"You're traveling — pulled this outfit from your saved plan for {dest}."}
        )

    # ── Weather rationale ─────────────────────────────────────────────────────
    temp = weather.get("temperature_c")
    if weather.get("is_raining"):
        bullets.append({"type": "weather", "text": "Rain expected — picked layers you can shed indoors."})
    elif weather.get("is_cold"):
        if temp is not None:
            bullets.append(
                {"type": "weather", "text": f"Cold day ({round(temp)}°C) — chose warmer fabrics and outerwear."}
            )
        else:
            bullets.append({"type": "weather", "text": "Cold day — chose warmer fabrics and outerwear."})
    elif weather.get("is_hot"):
        if temp is not None:
            bullets.append({"type": "weather", "text": f"Warm day ({round(temp)}°C) — kept it light and breathable."})
        else:
            bullets.append({"type": "weather", "text": "Warm day — kept it light and breathable."})
    ...
    if with_counts:
        ...
        if worn <= 2:
            ...
            bullets.append(
                {"type": "wear_balance", "text": f"Featured your {name} — you've only worn it {worn}× recently."}
            )
    ...
    if (weather.get("is_cold") or weather.get("is_raining")) and not has_outerwear:
        bullets.append(
            {"type": "weather", "text": "No outerwear in your wardrobe matched today — consider adding a jacket."}
        )
...
def _build_outfit_transitions(events: list[dict], wardrobe: list[dict], weather: dict | None) -> list[dict]:
    if not events:
        return []
...
def _today_events(user):
    ...
    return list(
        CalendarEvent.objects.filter(user=user, start_time__date=today).values(
...
def _active_trip_day_plan(user, target_date: datetime.date):
    ...
    trips = (
        Trip.objects.filter(user=user, start_date__lte=target_date, end_date__gte=target_date)
...
def run_daily_look(user, input_data: dict) -> dict:
    ...
    if trip and day_plan:
        ...
        if plan_ids:
            valid_ids = list(
                ClothingItem.objects.filter(id__in=plan_ids, user=user, is_active=True).values_list("id", flat=True)
            )
            if valid_ids:
    ...
    if output is None:
        ...
        if _has_mistral():
            output = _daily_look_mistral(user, wardrobe, events, weather, required_formality, cultural=cultural)
        else:
            output = _daily_look_stub(
                wardrobe, weather, required_formality, style_profile=style_profile, cultural=cultural
            )
    ...
    if item_ids:
        ...
        OutfitItem.objects.bulk_create([OutfitItem(outfit=rec, clothing_item=item, role="main") for item in items])
...
def _daily_look_cultural(user, today: datetime.date, weather: dict) -> dict | None:
    ...
    """
    destination = getattr(user, "location_name", None) or (
...
def _daily_look_stub(wardrobe, weather, required_formality, style_profile=None, cultural: dict | None = None) -> dict:
    ...
    if cultural:
        from ritha.services.recommendation_engine import (
            _cultural_hard_filters,
            _wardrobe_passes_cultural_filter,
        )
...
def _daily_look_mistral(user, wardrobe, events, weather, required_formality, cultural: dict | None = None) -> dict:
    ...
    if forbidden_tags or hard.get("reasons"):
        rule_lines = (
            "\n".join(f"- {r.get('description', r.get('rule_type', 'rule'))}" for r in hard.get("reasons", []))
        ...
        if forbidden_tags:
            cultural_inline = (
                f"- Hard rule: never pick an item whose name or tags contain any of: {sorted(forbidden_tags)}"
            )
...
def run_weekly_looks(user, input_data: dict) -> dict:
    ...
    from outfits.models import OutfitItem, OutfitRecommendation
    from ritha.services.event_classifier import dominant_formality
    from ritha.services.weather import _fallback, get_weather_forecast
    from wardrobe.models import ClothingItem
...
def _weekly_day_mistral(user, wardrobe, events, weather, required_formality, day_date, used_item_ids, day_idx) -> dict:
    from ritha.services.mistral_client import chat_json
...
def run_cultural_advisor(user, input_data: dict) -> dict:
    ...
    try:
        ...
        logging.getLogger(__name__).warning("Weather lookup failed for %s: %s", weather_query, exc)
        weather = None

    rules = list(
        CulturalRule.objects.filter(country__iexact=country).values(
...
def _generate_destination_highlights_ai(country: str, city: str, weather: dict | None = None) -> list[dict]:
    ...
    cache_key = f"cultural_highlights:{country.lower().strip()}:{city.lower().strip()}:{iso_week}:{weather_bucket}"
    ...
    try:
        ...
        logging.getLogger(__name__).warning("Destination highlights generation failed for %s: %s", place, exc)
        return []
...
def _consolidate_rules(rules: list[dict]) -> list[dict]:
    ...
    for r in rules:
        ...
        if desc and desc.lower() not in existing["description"].lower():
            existing["description"] = existing["description"] + " " + desc if existing["description"] else desc
...
def _match_wardrobe_against_rules(
    user, country: str, city: str, rules: list[dict], highlights: list[dict] | None = None, weather: dict | None = None
):
    ...
    version = ClothingItem.objects.filter(user=user).aggregate(v=Max("updated_at"))["v"]
    ...
    rules_text = (
        "\n".join(
            f"- [{r.get('severity', 'info')}] {(r.get('place_name') or 'general').strip()}: {r.get('description', '')}"
            for r in rules
        )
        or "(none)"
    )
    ...
    highlights_text = (
        "\n".join(
            f"- {h['type'].upper()}: {h['name']} ({h.get('when', 'year-round')}) — {h.get('clothing', '')}"
            for h in highlights
        )
        or "(none)"
    )
    ...
    try:
        ...
        logging.getLogger(__name__).warning("Wardrobe matching failed for %s: %s", place, exc)
        return [], []
    ...
    cache.set(cache_key, {"matches": wardrobe_matches, "gaps": gaps}, timeout=60 * 60 * 24 * 7)  # 7 days
...
def _generate_cultural_brief_ai(country: str, city: str, month: int, weather: dict | None = None) -> dict | None:
    ...
    cache_key = f"cultural_brief:{country.lower().strip()}:{city.lower().strip()}:{month}:{bucket}"
    cached = cache.get(cache_key)
    if cached is not None:
        return cached
    ...
    try:
        ...
        logging.getLogger(__name__).warning("Cultural advisor AI generation failed for %s: %s", place, exc)
        return None
...
def run_outfit_planner(user, input_data: dict) -> dict:
    ...
    for i in range(days_count):
        ...
        day_items = []
        if tops:
            day_items.append(tops[i % len(tops)])
        if bottoms:
            day_items.append(bottoms[i % len(bottoms)])
        if outerwear and weather.get("is_cold"):
            day_items.append(outerwear[i % len(outerwear)])
        if footwear:
            day_items.append(footwear[i % len(footwear)])
        # Add activewear on alternating days if available
        if activewear and i % 2 == 0:
            day_items.append(activewear[i % len(activewear)])
...
def _merge_wardrobe_across_cities(cities: list, results: list) -> list:
    ...
    for city, rec in zip(cities, results, strict=False):
...
def run_smart_recommend(user, input_data: dict) -> dict:
    ...
    if len(cities) > 1:
        ...
        return {
            "multi_city": True,
            "country": country,
            "cities": [{"city": c, "recommendation": r} for c, r in zip(cities, results, strict=False)],
