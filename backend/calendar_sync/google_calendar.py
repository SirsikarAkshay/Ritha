...
def get_google_email(creds_dict: dict) -> str | None:
...
def sync_events(user, days_behind: int = 7, days_ahead: int = 60) -> dict:
    ...
    try:
        ...
        logger.info(
            "Google sync for %s: +%d created, ~%d updated, %d errors", user.email, created, updated, len(errors)
        )
...
def revoke_access(user) -> bool:
    ...
    try:
        ...
        if token:
            req.post("https://oauth2.googleapis.com/revoke", params={"token": token}, timeout=5)
    except Exception as exc:
