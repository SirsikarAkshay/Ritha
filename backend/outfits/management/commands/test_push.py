...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            raise CommandError(f"User not found: {email}") from None

        token = user.device_push_token
        if not token:
            raise CommandError(
                f"{email} has no device token registered. Open the app → Profile → enable push notifications first."
            )
        ...
        if result["status"] == "sent":
            self.stdout.write(self.style.SUCCESS(f"Push sent — message_id: {result.get('message_id')}"))
