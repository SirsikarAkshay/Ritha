...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--user", default=None, help="Limit to a single user email")
        parser.add_argument("--dry-run", action="store_true", help="Print what would happen without creating records")
        ...
        for user in users:
            ...
            try:
                ...
                if result.get("status") in ("stub", "ai", "no_wardrobe"):
                    ...
                    if options["verbosity"] >= 2:
                        self.stdout.write(self.style.SUCCESS(f"  ✅ {user.email} — rec #{rec_id}"))
