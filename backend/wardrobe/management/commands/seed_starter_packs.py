...
class Command(BaseCommand):
    ...
    def handle(self, *args, reset=False, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"\n✓ Seeded {len(regions)} regions, {created_count} new items, {updated_count} updated."
            )
        )
        ...
        for region in region_map.values():
            for gender, _ in StarterPackItem.GENDER_CHOICES:
                n = StarterPackItem.objects.filter(region_cluster=region, gender=gender, is_default=True).count()
                opt = StarterPackItem.objects.filter(region_cluster=region, gender=gender, is_opt_in=True).count()
                if n or opt:
                    self.stdout.write(f"  {region.code:>22} / {gender:<10} → {n} default, {opt} opt-in")
