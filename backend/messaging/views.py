...
class ConversationListView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Conversation.objects.filter(Q(user_a=me) | Q(user_b=me)).order_by("-updated_at")
...
class ConversationOpenView(APIView):
    ...
    def post(self, request):
        ...
        if not Connection.are_connected(request.user, target):
            return Response(
                {"error": {"code": "not_connected", "message": "You can only message users you are connected with."}},
                status=status.HTTP_403_FORBIDDEN,
            )
...
class SendMessageView(APIView):
    ...
    def post(self, request, pk):
        ...
        if not Connection.are_connected(request.user, other):
            return Response(
                {"error": {"code": "not_connected", "message": "You are no longer connected with this user."}},
                status=status.HTTP_403_FORBIDDEN,
            )
