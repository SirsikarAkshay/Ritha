...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const loc = user?.location_name || 'Zurich'
    weatherApi.byLocation(loc).then(setWx).catch(() => {})  // falls back gracefully
  }, [])

  // Load today's recommendation
  useEffect(() => {
    outfits.daily().then(setRec).catch(() => {}).finally(() => setLoadingRec(false))
    fetch('/api/calendar/status/', { headers: { Authorization: `Bearer ${localStorage.getItem('gg_access')}` } })
  ...
  const syncNow = async () => {
    setSyncing(true)
    try {
      await fetch('/api/itinerary/events/sync/', {
        method: 'POST',
        headers: { Authorization: `Bearer ${localStorage.getItem('gg_access')}` },
      })
      const date = new Date().toISOString().split('T')[0]
      const data = await itinerary.events.list({ date })
  ...
  return (
    <div>
      ...
      {/* Calendar sync nudge */}
      {calStatus && !calStatus.google?.connected && !calStatus.apple?.connected && !calStatus.outlook?.connected && (
        <div className="alert alert-info fade-up fade-up-delay-3" style={{ marginTop:16, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <span>ℹ Connect a calendar to get smarter outfit suggestions.</span>
          <a href="/calendar" className="btn btn-ghost btn-sm" style={{ marginLeft:12, whiteSpace:'nowrap' }}>Set up →</a>
        </div>
      )}
      {calStatus && (calStatus.google?.connected || calStatus.apple?.connected || calStatus.outlook?.connected) && (
        <div style={{ display:'flex', justifyContent:'flex-end', marginTop:8 }} className="fade-up fade-up-delay-3">
          <button className="btn btn-ghost btn-sm" onClick={syncNow} disabled={syncing}>
            {syncing ? <><span className="spinner" style={{width:12,height:12}}/> Syncing…</> : '↻ Sync calendar'}
          </button>
        </div>
      )}
      ...
      <div className="grid-3 fade-up fade-up-delay-3" style={{ marginTop: '24px' }}>
        {[
          { icon: '👔', label: 'Add wardrobe item', to: '/wardrobe' },
          { icon: '✈',  label: 'Plan a trip',       to: '/trips' },
          { icon: '🌍', label: 'Cultural guide',     to: '/cultural' },
        ].map(({ icon, label, to }) => (
          <Link key={to} to={to} className="card" style={{ display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer', textDecoration: 'none' }}>
