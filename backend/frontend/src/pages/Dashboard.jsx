...
import { outfits, agents, weather, itinerary } from '../api/client'
...
export default function Dashboard() {
  ...
  const loadOutfit = async () => {
    try {
      const { data } = await outfits.daily({ date: today })
      setOutfit(data)
  ...
  const loadEvents = async () => {
    try {
      const { data } = await itinerary.listEvents({ date: today })
  ...
  const loadWeather = async () => {
    try {
      const { data } = await weather.get({ location: 'Zurich' })
      setWx(data)
    } catch { setWx(null) }
  }
