...
export default function ItineraryPage() {
  ...
  const setDateFilter = (d) => setSearchParams({ date: d })
  ...
  const syncCalendar = async () => {
    ...
    setError('')
    try {
      // Call the real sync endpoint which routes to Google/Apple based on connection
      const res = await fetch('/api/itinerary/events/sync/', {
        method: 'POST',
        headers: { Authorization: `Bearer ${localStorage.getItem('gg_access')}` },
      }).then(r => r.json())

      if (res.status === 'no_calendars_connected') {
        setError('No calendars connected. Go to Calendar settings to connect Google or Apple Calendar.')
      } else if (res.status === 'synced') {
        setError(`Synced — ${res.created} new, ${res.updated} updated events.`)
        await loadEvents()
      } else {
        setError(res.message || 'Sync initiated.')
      }
    } catch (e) {
      setError(e.message)
    } finally {
