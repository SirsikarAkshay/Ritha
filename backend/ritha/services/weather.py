...
def get_weather(lat: float, lon: float, date: datetime.date | None = None) -> dict:
    """
...
def get_weather_for_location(location_str: str, date: datetime.date | None = None) -> dict:
    """
...
def get_weather_forecast(location_str: str, start_date: datetime.date, end_date: datetime.date) -> list:
    ...
    try:
        ...
        if not results:
            ...
            return [
                _fallback(f"Location not found: {location_str}", start_date + datetime.timedelta(days=i))
    ...
    if len(forecasts) < num_days:
        last_forecast_date = (
            datetime.date.fromisoformat(forecasts[-1]["date"]) if forecasts else start_date - datetime.timedelta(days=1)
        )
...
def get_climatology_forecast(
    lat: float, lon: float, start_date: datetime.date, end_date: datetime.date, loc_name: str = ""
) -> list:
    """
    ...
    agg = {i: {"tmax": [], "tmin": [], "precip": [], "precip_p": [], "wmo": [], "wind": []} for i in range(num_days)}
    ...
    for yoff in range(years_back):
        ...
        for idx in range(min(len(times), num_days)):
            if idx < len(tmax_l) and tmax_l[idx] is not None:
                agg[idx]["tmax"].append(tmax_l[idx])
            if idx < len(tmin_l) and tmin_l[idx] is not None:
                agg[idx]["tmin"].append(tmin_l[idx])
            if idx < len(pr_l) and pr_l[idx] is not None:
                agg[idx]["precip"].append(pr_l[idx])
            if idx < len(wmo_l) and wmo_l[idx] is not None:
                agg[idx]["wmo"].append(int(wmo_l[idx]))
            if idx < len(wind_l) and wind_l[idx] is not None:
                agg[idx]["wind"].append(wind_l[idx])
    ...
    if not any(agg[i]["tmax"] for i in range(num_days)):
        return [_fallback("climatology unavailable", start_date + datetime.timedelta(days=i)) for i in range(num_days)]
