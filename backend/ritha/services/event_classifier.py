...
_RULES = [
    ...
    # Travel
    (r"\b(flight|fly|airport|train|depart|arrive|layover|check.?in|check.?out|hotel|airbnb)\b", "travel", "casual"),
    # Weddings / ceremonies
    (r"\b(wedding|ceremony|reception|engagement|gala|ball|black.?tie|white.?tie|prom)\b", "wedding", "formal"),
    # Interviews
    (r"\b(interview|job interview|hiring|recruitment)\b", "interview", "smart"),
    ...
    # Dates
    (r"\b(date night|date with|romantic)\b", "date", "smart"),
    # Catch-alls for meetings
    (r"\b(meeting|call|zoom|teams|google meet|webex|conference)\b", "internal_meeting", "casual_smart"),
]
...
def dominant_formality(events: list) -> str:
    ...
    best = max(ranks)
    return {v: k for k, v in {"activewear": 0, "casual": 1, "casual_smart": 2, "smart": 3, "formal": 4}.items()}.get(
        best, "casual"
    )
