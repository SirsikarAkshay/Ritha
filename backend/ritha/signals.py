...
def handle_outfit_feedback(sender, instance, created, **kwargs):
    ...
    accepted_categories = list(
        OutfitItem.objects.filter(outfit=instance).values_list("clothing_item__category", flat=True)
    )
    accepted_formalities = list(
        OutfitItem.objects.filter(outfit=instance).values_list("clothing_item__formality", flat=True)
    )
